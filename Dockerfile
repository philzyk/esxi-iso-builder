...
# Set non-interactive environment for apt installation
...
RUN curl -Lo /tmp/${DOTNET_PACKAGE} ${DOTNET_PACKAGE_URL} \
    && mkdir -p ${DOTNET_ROOT} \
...
# Install PowerCLI via PSGallery
...
RUN python3.7 /tmp/get-pip.py \
    ...
    && rm /tmp/get-pip.py \
    && pwsh -Command Set-PowerCLIConfiguration -Scope User -ParticipateInCEIP \$${VMWARECEIP} -Confirm:\$false \
    && pwsh -Command Set-PowerCLIConfiguration -PythonPath /usr/bin/python3.7 -Scope User -Confirm:\$false

# Set interactive mode back after build
