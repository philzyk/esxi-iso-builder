...
# Detect the OS type (Windows or Linux)
function Get-OSType {
    <#
    .SYNOPSIS
    Detects the operating system type (Windows or Linux).
    .DESCRIPTION
    This function detects the operating system type and returns the appropriate path separator.
    .OUTPUTS
    The path separator based on the detected OS type.
    #>
    if ($IsWindows) {
        return "\"
    } else {
...
# Function to get the output file path based on the OS type
function Get-OutputFilePath {
    <#
    .SYNOPSIS
    Generates the output file path based on the OS type and the provided parameters.
    .DESCRIPTION
    This function generates the output file path for the customized ISO or Offline bundle, considering the OS type.
    .PARAMETER OutDir
    The output directory specified by the user.
    .PARAMETER MyProfile
    The customized Imageprofile.
    .PARAMETER OZip
    A switch indicating whether to create an Offline bundle or an ISO.
    .OUTPUTS
    ...
    $pathSeparator = Get-OSType
    if ($OZip) {
        $outFile = $OutDir + $pathSeparator + $MyProfile.Name + ".zip"
    } else {
        $outFile = $OutDir + $pathSeparator + $MyProfile.Name + ".iso"
    }
    return $outFile
}

# Function to check if the output file directory exists and create it if necessary
function Test-OutputDirectory {
    <#
    .SYNOPSIS
    Ensures the output directory exists, creating it if necessary.
    .DESCRIPTION
    This function checks if the output directory exists, and creates it if it doesn't.
    .PARAMETER OutDir
    The output directory specified by the user.
    .OUTPUTS
    None
    ...
    $pathSeparator = Get-OSType
    $outDirParts = $OutDir -split $pathSeparator
    $currentDir = $outDirParts[0]

    for ($i = 1; $i -lt $outDirParts.Length; $i++) {
        $currentDir += $pathSeparator + $outDirParts[$i]
